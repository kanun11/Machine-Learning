...
data= np.loadtxt('/Users/kanun/Desktop/ML_project/data.txt') # to load text saved usig num
...
final_layer_input = tf.add(tf.matmul(second_layer_output, w_out), b_out)
final_output = final_layer_input

finaloutput = tf.square(final_output)
constraint = tf.math.add(1.0,finaloutput)
Y_predicted = tf.math.divide(1.6708, constraint, name = 'predict')
...
with tf.Session() as sess:
    ...
    model2 = saver.save(sess, '/Users/kanun/Desktop/ML_project/python_code/constraint/cons')
...
f= open("/Users/kanun/Desktop/ML_project/python_code/constraint/train.txt","w")
data = train_predict
np.savetxt(f, data,fmt='%.8f' )
f.close()

test_predict = predict_test
g= open("/Users/kanun/Desktop/ML_project/python_code/constraint/test.txt","w")
data = test_predict
...
ax1.scatter(y_train,predicted_value,size,color = 'red',label="training data")
